/* This program is for section 5.2.2 of Jacques' 2011 book chapter, page 113.
   https://omlc.org/software/mc/Jacques2011_MonteCarlo_Welch&VanGemert.pdf

   It predicts where a photon should be launched at a tissue to mimic a 
   circular flat field beam.
...
#define N_BINS  (35)      /* number of bins * bin width = range of x axis */
#define BIN_WIDTH (0.0327) /* this is "ds" in cm. reduce this for better resolution */
...
int main () 
{
	...
	{
		myRand = rand();
		myScaledRand = myRand/RAND_MAX;  /* the +1 is the magic b/c of the log */
		s1 = sqrt(myScaledRand);
	...
	printf("Min and max values of S1 modeled as sqrt(RAND): %f %f\n", minS1, maxS1);
	return 0;
}
