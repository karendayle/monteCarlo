...
for iLoop = 1:1 %length(infile)
    ...
    figure
    grid on
    for i = 1:2000 %length(x) % takes a long time length(x)
        switch myColor(i)
            case 0
        ...
        %plot3(x(i),y(i),z(i),'.','Color',plotColor); does not allow Size
        %of symbol to be changed
        scatter3(x(i),y(i),z(i),200,plotColor,'filled');
        hold on;
        pause(0.001);
    end;
