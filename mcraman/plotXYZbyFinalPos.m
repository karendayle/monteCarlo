...
data = load(char('skin3min_STEPS.txt'), '-ascii');
maxZ = data(:,3); % column 3

infile = {'skin3min-ENDPOS-PH7.txt'}
%     'skin3min-ENDPOS-PH4.txt', ...
...
%             x(i),y(i),row,col);
        ...
        if row > 0 && col > 0
            ...
            switch myColor(i)
                ...
                case 3 % change to 1584 cm-1    
                    wL1584bins(row,col) = wL1584bins(row,col) + 1;        
            ...
            % TO DO only for the row and col you want to look at. Not all
...
%     grid on
    ...
    title('Photons of all wavelengths');
    xlabel('bin number * 0.1 mm spacing');
    ylabel('bin number * 0.1 mm spacing');
    ...
    title('Photons with 1072 cm-1 wavenumber\n');
    xlabel('bin number * 0.1 mm spacing');
    ylabel('bin number * 0.1 mm spacing');    
    ...
    title('Photons with 1430 cm-1 wavenumber');
    xlabel('bin number * 0.1 mm spacing');
    ylabel('bin number * 0.1 mm spacing');    
    fprintf('Photons with 1584 cm-1 wavenumber\n');
    for j = 1:10
        for i = 1:10
            fprintf('%d ', wL1584bins(i,j));
        end
        fprintf('\n');
    end
    figure
    grid on
    heatmap(wL1584bins', 'Colormap', parula); % transpose to flip column major order to row
    title('Photons with 1584 cm-1 wavenumber');
    xlabel('bin number * 0.1 mm spacing');
    ylabel('bin number * 0.1 mm spacing');    
    ...
    title('Photons with 1702 cm-1 wavenumber');    
    xlabel('bin number * 0.1 mm spacing');
    ylabel('bin number * 0.1 mm spacing');    
    ...
    title('Photons with other wavenumbers');
    xlabel('bin number * 0.1 mm spacing');
    ylabel('bin number * 0.1 mm spacing');
end

% Plot heatmaps of each square of the grid
figure
grid on
heatmap(wLotherbins', 'Colormap', parula); % transpose to flip column major order to row
title('Photons with other wavenumbers');
